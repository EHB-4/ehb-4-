...
export class FeatureExtractor {
  ...
  static extractTransactionHistoryFeatures(transactions: any[]): TransactionHistoryFeatures {
    ...
    return {
      ...
      totalReceived: successful.filter(t => t.type === 'sale').reduce((sum, t) => sum + t.amount, 0),
  ...
  private static calculateKYCTime(submittedAt: Date, approvedAt: Date): number {
    ...
    return Math.floor((new Date(approvedAt).getTime() - new Date(submittedAt).getTime()) / (1000 * 60 * 60 * 24));
  }
  ...
  private static calculateTransactionsPerMonth(transactions: any[]): number {
    ...
    const months = (lastTransaction.getTime() - firstTransaction.getTime()) / (1000 * 60 * 60 * 24 * 30);
  ...
  private static calculateAverageDaysBetweenTransactions(transactions: any[]): number {
    ...
    for (let i = 1; i < sortedTransactions.length; i++) {
      const days = (new Date(sortedTransactions[i].createdAt).getTime() - new Date(sortedTransactions[i - 1].createdAt).getTime()) / (1000 * 60 * 60 * 24);
      totalDays += days;
    }
  ...
  private static analyzeTransactionTimePattern(transactions: any[]): 'regular' | 'irregular' | 'seasonal' | 'random' {
    // Simplified analysis - in practice, use more sophisticated time series analysis
    const intervals = [];
    for (let i = 1; i < transactions.length; i++) {
      const interval = new Date(transactions[i].createdAt).getTime() - new Date(transactions[i - 1].createdAt).getTime();
  ...
  private static analyzeTransactionValuePattern(transactions: any[]): 'consistent' | 'variable' | 'increasing' | 'decreasing' {
  ...
  private static analyzeTransactionCategoryPattern(transactions: any[]): 'diverse' | 'focused' | 'random' {
...
export class AIScoringEngine {
  ...
  private calculateRiskScore(componentScores: any): number {
    ...
    let score = componentScores.financialScore * weights.financialScore +
  ...
  private calculateFraudScore(componentScores: any): number {
    ...
    let score = componentScores.behavioralScore * weights.behavioralScore +
                componentScores.securityScore * weights.securityScore;
    
    // Add fraud indicator penalties
    score += componentScores.riskIndicators.fraudScore * 0.30;
