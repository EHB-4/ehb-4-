...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check rate limit
    const { success, limit, reset, remaining } = await ratelimit.limit(`openai_${session.user.id}`);
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch OpenAI requests' }, { status: 500 });
  }
}
