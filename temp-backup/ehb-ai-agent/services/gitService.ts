...
export class GitService {
  ...
  private constructor() {
    ...
    console.log(
      `🚀 GitService initialized in ${this.config.fastMode ? 'ULTRA-FAST' : 'normal'} mode`
    );
    if (this.config.fastMode) {
      console.log(
        `⚡ Performance optimizations: ${this.cpuCount} cores, ${this.config.useNoVerify ? 'hooks disabled' : 'hooks enabled'}, ${this.config.useAtomicPush ? 'atomic push' : 'standard push'}`
      );
    }
  }
  ...
  private async executeCommand(command: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      exec(command, { maxBuffer: this.config.maxBuffer }, (error, stdout, stderr) => {
        ...
        if (execTime > 1000) {
          console.log(
            `⏱️ Git command took ${execTime}ms: ${command.slice(0, 50)}${command.length > 50 ? '...' : ''}`
          );
        }
  ...
  async commitAndPush(message: string): Promise<boolean> {
    ...
    try {
      ...
      if (this.config.autoPush) {
          ...
          if (error.message && typeof error.message === 'string') {
            if (error.message.includes('no upstream branch')) {
              ...
              await this.executeCommand(
                `git push origin ${this.config.branch}${this.config.useNoVerify ? ' --no-verify' : ''}`
              );
            } else {
              throw error;
            }
        ...
        const execTime = Date.now() - startTime;
        console.log(
          `✅ Changes pushed to ${this.config.branch} at ${new Date().toLocaleTimeString()} (${execTime}ms)`
        );
      } else {
