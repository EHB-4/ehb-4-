...
export const TaskPrioritizer: React.FC = () => {
  ...
  useEffect(() => {
    Object.entries(state.agentControls).forEach(([agentId, control]) => {
      const urgentTasks = control.tasks.filter(
        t => t.description?.toLowerCase().includes('urgent') && t.status !== 'completed'
      );
      const otherTasks = control.tasks.filter(
        t => !t.description?.toLowerCase().includes('urgent') && t.status !== 'completed'
      );
      const completedTasks = control.tasks.filter(t => t.status === 'completed');
      const newOrder = [...urgentTasks, ...otherTasks, ...completedTasks];
      if (
        JSON.stringify(newOrder.map(t => t.id)) !== JSON.stringify(control.tasks.map(t => t.id))
      ) {
