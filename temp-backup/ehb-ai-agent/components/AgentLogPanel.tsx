    ...
    <div className="h-full bg-white rounded-lg shadow-lg p-4">
      ...
      <div className="space-y-2 max-h-[calc(100vh-20rem)] overflow-y-auto">
        {filteredLogs.map(log => (
          <div key={log.id} className={`p-3 rounded-lg border ${getStatusColor(log.type)}`}>
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {log.module && <span className="text-sm ml-2 opacity-75">[{log.module}]</span>}
                  </div>
                  <span className="text-xs opacity-75">{log.timestamp.toLocaleTimeString()}</span>
                </div>
                ...
                {log.details && (
                  <div className="mt-2 text-xs bg-white bg-opacity-50 p-2 rounded">
                    ...
                    {log.details.suggestion && (
                      <p className="mt-1 text-blue-600">Suggestion: {log.details.suggestion}</p>
                    )}
                  </div>
                )}
              </div>
