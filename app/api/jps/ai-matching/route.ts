...
class AIMatchingEngine {
  ...
  static calculateExperienceCompatibility(jobRequirements: string[], candidateExperience: number): number {
  ...
  static getMatchingAnalysis(jobId: string, candidateId: string) {
    ...
    const experienceScore = this.calculateExperienceCompatibility(job.requirements, candidate.experience);
    const locationScore = this.calculateLocationCompatibility(job.location, candidate.preferredLocation);
  ...
  static generateRecommendations(overallScore: number, skillScore: number, experienceScore: number): string[] {
    ...
    if (overallScore >= 90) {
      ...
      recommendations.push('Moderate match. Consider for interview if other candidates are limited.');
    } else {
